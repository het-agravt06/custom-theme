 ...
 */
if (!function_exists('cinema_house_is_plugin_installed')) {
    function cinema_house_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('cinema_house_is_plugin_activated')) {
    function cinema_house_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_cinema_house_install_and_activate_plugins', 'cinema_house_install_and_activate_plugins');
add_action('wp_ajax_nopriv_cinema_house_install_and_activate_plugins', 'cinema_house_install_and_activate_plugins');
add_action('wp_ajax_cinema_house_rplugin_activation', 'cinema_house_rplugin_activation');
add_action('wp_ajax_nopriv_cinema_house_rplugin_activation', 'cinema_house_rplugin_activation');
...
function cinema_house_all_plugins_active() {
...
function cinema_house_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('cinema_house_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_cinema_house_plugin_installed($plugin_slug)) {
    ...
    $redirect_url = admin_url('themes.php?page=cinema-house-pro');
    echo json_encode([
        'success' => true,
        'data' => [
...
function is_cinema_house_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_cinema_house_dismissed_notice_handler', 'cinema_house_ajax_notice_dismiss_function');

function cinema_house_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'cinema_house_welcome_nonce')) {
...
function cinema_house_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        ...
        if ($cinema_house_current_screen && $cinema_house_current_screen->id !== 'appearance_page_cinema-house-pro') {
            ?>

            <div class="cinema-house-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="cinema-house-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($cinema_house_theme->get('Name')); ?></p>
                    ...
                    <div class="notice-buttons-box">
                        <a class="button-primary getstarted" href="<?php echo esc_url( admin_url( 'themes.php?page=cinema-house-pro' ) ); ?>" target="_blank"><?php esc_html_e('Theme Informations', 'cinema-house'); ?></a>
                        ...
                        <a class="button-primary buynow" href="<?php echo esc_url( CINEMA_HOUSE_BUY_PRO ); ?>" target="_blank"><?php esc_html_e('Get Pro Theme', 'cinema-house'); ?></a>
                    </div>
                </div>
            </div>
            <?php
