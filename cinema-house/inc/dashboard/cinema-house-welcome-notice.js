(function ($) {
    ...
    $("#install-activate-button").on("click", function (e) {
        ...
        var activationData = {
            action: "cinema_house_install_and_activate_plugins",
            nonce: cinema_house_localize.nonce,
        };

        $.post(cinema_house_localize.ajax_url, activationData, function (response) {
    ...
    $(document).on('click', '.notice-info .notice-dismiss', function () {
        ...
        $.ajax({
            type: 'POST',
            url: cinema_house_localize.ajax_url,
            data: {
                action: 'cinema_house_dismissed_notice_handler',
                type: type,
                wpnonce: cinema_house_localize.dismiss_nonce
            },
